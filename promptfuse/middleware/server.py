...
class _LogWriter:
    ...
    async def flush(self) -> None:
        async with self._lock:
            ...
            self._buffer.clear()
        # Write outside the lock to avoid blocking new events.
...
class PromptFuseServer:
    ...
    # Which message roles are eligible for compression/unification.
    ...
    def _process_messages(
        ...
        Returns the transformed message list and the pipeline result from the
        *first* compressible message (typically the system prompt) for logging.
        """
        ...
        for msg in messages:
            if msg.role in self._COMPRESSIBLE_ROLES and msg.content.strip():
                ...
                # Keep the first processed result for logging/stats.
                if pipeline_result is None:
                    pipeline_result = processed
            else:
                forwarded.append({"role": msg.role, "content": msg.content})

        # If nothing was compressible (no system prompt), run the last user
        # message through the pipeline so we still get *some* unification
        # benefit for prefix caching.
        if pipeline_result is None:
        ...
        # Final fallback: create a no-op result so callers don't need to
        ...
        *,
        ...
        async def iterator():
            try:
                ...
                with suppress(Exception):
                    await client.aclose()
                # Fire-and-forget log; don't block the stream teardown.
                with suppress(Exception):
                    await self._record_and_log(
                        endpoint=endpoint, request_id=request_id,
                        processed=processed, request_meta=request_meta,
